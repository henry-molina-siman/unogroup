...
>
> **Relación con la Bitácora de Diseño:** las preguntas de negocio y las decisiones de arquitectura/contrato (`F1`–`F20`, `A1`–`A7`, `C1`–`C7`, etc.) viven en `HUENSA-001_Diseno_Bitacora_Decisiones_Modulo_Integracion_Ensamble.md` y no se repiten aquí — esta bitácora solo referencia esos IDs cuando un hallazgo de código los afecta. Lo que sí vive aquí, y no en la de Diseño, es todo lo que solo tiene sentido al nivel de código: nombres de clase, workarounds concretos, archivos/paquetes que faltan, y el detalle de qué se auditó, cuándo y contra qué repositorio.
...
| v3.1 — migración `transacciones[]` + reconciliación 2026-07-30 | El documento de Implementación migró a nivel de código al contrato v3 (`ResultadoSolicitud.transacciones[]`, ver Bitácora de Diseño v3.2): `ensamble_bitacora_partner` recreada (`V4__recreate_ensamble_bitacora_partner_v3.sql`), `ensamble_solicitud` pierde `payload_partner`/`nombre_archivo` (`V5__drop_...sql`), `BitacoraPartner`/`SolicitudEnsamble` reescritas. Esta Bitácora no se había actualizado en su momento — quedó describiendo el problema de `NOT NULL` de v2 como abierto (§2.1) aun cuando ya estaba resuelto por el nuevo contrato. Se pone al día en esta reconciliación, junto con la confirmación de que `unogroup-app` implementa el interceptor de captura vía `CapturingFeignClient` — ver §2.1, §2.2 y §4 (ítem 3). |
...
**Discrepancia de `NOT NULL` en `ensamble_bitacora_partner` — resuelta 2026-07-29 vía contrato v3 (afecta Implementación §2, §3; Diseño §2.4, §2.6.1, F8):**
`url` y `metodo_http` eran `NOT NULL` en el DDL y en la entidad `BitacoraPartner`, pero ni el JSON de ejemplo del callback ni el `IntentoDto` real del código llevaban una URL o un método HTTP por intento — el callback nunca los proveía. `SolicitudEnsambleService` lo resolvía con un workaround no documentado: hardcodeaba `url = "N/D"` y derivaba `metodo_http` de `tipo_peticion`. **Resuelto:** el contrato v3 (`ResultadoSolicitud.transacciones[]`, reemplaza `intentos[]`) hace obligatorios `request.method`/`request.url` en cada transacción — `ensamble_bitacora_partner` se recreó (`V4__recreate_ensamble_bitacora_partner_v3.sql`) con `request_method`/`request_url` `NOT NULL` sin necesidad de workaround; `BitacoraPartner`/`IntentoDto` reescritas (`IntentoDto` retirado, reemplazado por `TransaccionHttpDto`). Ver Implementación §2/§3.
...
**Interceptor de captura de transacciones HTTP — confirmado implementado (2026-07-30, afecta Implementación §1.4.6):**
`unogroup-app` implementa la captura de `transacciones[]` (contrato v3) vía `CapturingFeignClient`, un envoltorio único sobre el `Client` de Feign que registra method/url/headers/body (con enmascarado de valores sensibles en url/headers; el body queda sin enmascarar, limitación conocida de v3) tanto para la respuesta como para el error de cada llamada real (auth + upload). `service` solo anexa la metadata de negocio (`secuencia`/`proposito`/`esReintento`) antes de reportar el resultado en el callback. Esto cierra el ítem 3 de §4 — no se revisó el código fuente directamente para esta entrada, la confirmación viene de la copia de Implementación del equipo de UnoGroup.

## 2.3 Trade-off aceptado: enums duplicados sin módulo común (transversal a ambas apps)

Con `ensambles-common` eliminado (§1 de esta bitácora), los enums de dominio (`EstadoInterno`, `TrackingStatus`, `FlujoEnsamble`) ya no se comparten vía un `.jar` común — cada app define su propia copia local, con los mismos valores. **Riesgo aceptado conscientemente:** existe la posibilidad de que ambas copias se desincronicen si alguien cambia un valor en una app y olvida replicarlo en la otra. Se acepta este riesgo a cambio de independencia total de build entre los dos microservicios — la fuente de verdad real es el contrato OpenAPI (`HUENSA-001_openapi_V3.yaml`), no el código Java de ninguna de las dos apps.
...
| ~~3~~ | ~~Decidir si se agrega `url`/`metodoHttp` al contrato del callback, o se relaja el `NOT NULL` en `ensamble_bitacora_partner`~~ — **Resuelto vía contrato v3** (`transacciones[]`, obligatorios en el contrato) y confirmado implementado en `unogroup-app` vía `CapturingFeignClient` | `orquestador-app` (DDL, entidad, contrato del callback), `unogroup-app/client` | §2.1, §2.2 de esta bitácora; Diseño F8 |
